"""Output data structures for unlearning evaluation results."""
...
class UnlearningMetrics(BaseMetrics):
    unlearning_score: float = Field(
        ...
        description="Unlearning score using Dynamic SAE Guardrails methodology",
...
class UnlearningEvalOutput(
    ...
    """Evaluation of Dynamic SAE Guardrails for unlearning on WMDP datasets.
    
    Implements dynamic feature intervention that only applies clamping when
    the fraction of tokens with active features exceeds a threshold within
    each batch, allowing for more selective unlearning compared to static methods.
    """
